...
def compare_by_protein_id_result(request):
    ...
    if request.method == "POST":
        ...
        for protein in protein_list:
            try:
                similarity_info_queryset = Comparison.objects.get(protein_one=protein, protein_two=protein_compared)
            except:
                continue
            ...
            row_val.append({
                'ProtId': protein,
                'similarity': similarity_info_queryset.similarity_value,
                # 'class': hierarchy_result.Class_id,
